...
function rotateRight64(n, x) {
    return { high: (x.high >>> n) | (x.low << (32 - n)), low: (x.low >>> n) | (x.high << (32 - n)) };
}

function add64(a, b) {
    let low = (a.low + b.low) >>> 0;
    let high = (a.high + b.high + ((low < a.low) ? 1 : 0)) >>> 0;
    return { high, low };
}

function sigma0_512(x) {
    let rotr28 = rotateRight64(28, x);
    let rotr34 = rotateRight64(34, x);
    let rotr39 = rotateRight64(39, x);
    return { high: rotr28.high ^ rotr34.high ^ rotr39.high, low: rotr28.low ^ rotr34.low ^ rotr39.low };
}

function sigma1_512(x) {
    let rotr14 = rotateRight64(14, x);
    let rotr18 = rotateRight64(18, x);
    let rotr41 = rotateRight64(41, x);
    return { high: rotr14.high ^ rotr18.high ^ rotr41.high, low: rotr14.low ^ rotr18.low ^ rotr41.low };
}

function sigma0_256(x) {
    let rotr1 = rotateRight64(1, x);
    let rotr8 = rotateRight64(8, x);
    let shr7 = { high: 0, low: x.low >>> 7 };
    return { high: rotr1.high ^ rotr8.high ^ shr7.high, low: rotr1.low ^ rotr8.low ^ shr7.low };
}

function sigma1_256(x) {
    let rotr19 = rotateRight64(19, x);
    let rotr61 = rotateRight64(61, x);
    let shr6 = { high: 0, low: x.low >>> 6 };
    return { high: rotr19.high ^ rotr61.high ^ shr6.high, low: rotr19.low ^ rotr61.low ^ shr6.low };
}

function choice(x, y, z) {
    return { high: (x.high & y.high) ^ (~x.high & z.high), low: (x.low & y.low) ^ (~x.low & z.low) };
}

function majority(x, y, z) {
    return { high: (x.high & y.high) ^ (x.high & z.high) ^ (y.high & z.high), low: (x.low & y.low) ^ (x.low & z.low) ^ (y.low & z.low) };
}
...
function sha512(message) {
    ...
    return toHexStr64(H[0]) + toHexStr64(H[1]) + toHexStr64(H[2]) + toHexStr64(H[3]) +
           toHexStr64(H[4]) + toHexStr64(H[5]) + toHexStr64(H[6]) + toHexStr64(H[7]);
}
